...
- `line`/`bar` 不写列名时默认：第一列作 X 轴、其余所有数值列作序列。
...
数据列名取 key 值 (形如 `product[A]`)。**行数上限 50**, 超出则放弃转置。转置后 `@sum`/`@avg` 跳过。
...
波动消息。消息会进入**订阅推送**正文 (见 REPORT.md 订阅章节), 用于异动告警。
...
最新期该字段缺失或为 0 记为"数据缺失"; 表不足两行则不检测。

> 检测只产出消息, **不改变表格展示**。消息仅在订阅推送时随正文发出。
...
脚本 = **在服务器上执行任意 JS**, 仅 `report.manage:rw` 可写, **务必只在内网信任环境部署**。
